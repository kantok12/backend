...
router.get('/', async (req, res) => {
  try {
    ...
    const personalAsignado = req.query.personal_asignado;

    let query = supabase
      .from('lubricacion.tareas_programadas')
      .select(`
        *,
    ...
    if (search) {
      query = query.or(`observaciones.ilike.%${search}%, estado.ilike.%${search}%`);
    }
    ...
    if (personalAsignado) {
      query = query.eq('personal_asignado', personalAsignado);
    }
